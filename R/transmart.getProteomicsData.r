...
#-----------------------
transmart.getProteomicsData <- function(
  study.list    	=NA,
  protein.list				=NA,
  pathway				=NA,
  signature				=NA,
...
)
{
  ...
  if((!is.na(pathway) && !is.na(signature)) || (!is.na(pathway) && any(!is.na(protein.list))) || (!is.na(signature) && any(!is.na(peptide.list))))
  {
  ...
  if(!is.na(pathway) || any(!is.na(protein.list)) || any(!is.null(peptide.list)))
  {
    ...
    #Add the table joins.
    newTableJoins <- gsub("\n", "", "
                          INNER JOIN deapp.de_protein_annotation b ON a.protein_annotation_id = b.id 
    ...
    }else if(any(!is.na(protein.list)))
    {
      ...
      baseSQLWhereStatement <- paste(baseSQLWhereStatement," AND skt.KEYWORD_TERM IN (",searchWords,")",sep="")
    }else if(any(!is.null(peptide.list)))
    {
      searchWords <- paste("'",peptide.list,"'",sep="",collapse=",")
      baseSQLWhereStatement <- paste(baseSQLWhereStatement," AND b.peptide IN (",searchWords,")",sep="")
    }
  ...
  }else if(!is.na(signature))
  {
    ...
    newTableJoins <- gsub("\n", "", "
                          INNER JOIN 
                          (
                          select bbm.peptide, bbm.gpl_id, bbm.uniprot_id, bbm.id                       
                          ...
                          inner join deapp.de_protein_annotation bbm ON to_char(bbm.uniprot_id) = bm2.primary_external_id
                          WHERE          skt.KEYWORD_TERM IN (?) 
                          ) b ON a.protein_annotation_id = b.id  ")
  ...
  }else
  {
    #Add a SELECT DISTINCT to the select statement.
    baseSQLSelectStatement <- paste("SELECT DISTINCT ",baseSQLSelectStatement,sep=" ")	
    newTableJoins <- " INNER JOIN deapp.de_protein_annotation b ON a.protein_annotation_id = b.id "
    baseSQLTableStatement <- paste(baseSQLTableStatement,newTableJoins,sep="")
  }
  ...
  print("Sending Proteomics Query.")
  
  #Send the query to the server.
  rs <- dbSendQuery(tranSMART.DB.connection, finalQuery)
  
  print("Retrieving Proteomics Records.")
  ...
  if(data.pivot == TRUE)
  {
    ...
    relevantColumns <- c(pivotPatient,'LOG_INTENSITY','PEPTIDE','GPL_ID')
    
    #This is the formula we use to pivot the GEX data.
    pivotFormula <- paste("GPL_ID + PEPTIDE ~ ",pivotPatient)
    ...
    if(show.proteins == TRUE)
    {
      ...
      pivotFormula <- paste("UNIPROT_ID + GPL_ID + PEPTIDE ~ ",pivotPatient)
    }
  ...
  if(any(!is.null(platform.removeOnOverlap)))
  {
    ...
    probeData <- data.frame(table(dataToReturn$PEPTIDE))
    
    colnames(probeData) <- c('PEPTIDE','Freq')
    ...
    dataToReturn <- dataToReturn[which(!(dataToReturn$PEPTIDE %in% probeData$PEPTIDE & dataToReturn$GPL_ID %in% platform.removeOnOverlap)),]
